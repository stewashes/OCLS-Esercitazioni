...
m.addConstrs(
    (quicksum(x[i, j, 'prod1'] for j in connections[i]) - quicksum(x[j, i, 'prod1'] for j in connections[i]) == od[i,'prod1'] for i in Nodes), name='flow_prod1')

m.addConstrs(
    (quicksum(x[i, j, 'prod2'] for j in connections[i]) - quicksum(x[j, i, 'prod2'] for j in connections[i]) == od[i,'prod2'] for i in Nodes), name='flow_prod2')
...
m.write('/Users/silvia/Desktop/model_nd_gr2.lp')
